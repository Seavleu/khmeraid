...
export default function FilterPanel({ 
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md p-4 space-y-4 max-h-[70vh] overflow-y-auto">
      ...
      {onRadiusChange && hasUserLocation && (
        <div className="space-y-3 bg-blue-50 p-3 rounded-xl border-2 border-blue-200">
      ...
      <div className="bg-purple-50 border-2 border-purple-200 p-3 rounded-xl">
      ...
      {/* Type Filter */}
      <div className="space-y-2">
        <p className="text-sm font-medium text-gray-600">ប្រភេទជំនួយ</p>
        <div className="flex flex-wrap gap-2">
          {typeFilters.map(({ value, label, icon: Icon }) => (
            <Button
              key={value}
      ...
      {/* Status Filter */}
      <div className="space-y-2">
        <p className="text-sm font-medium text-gray-600">ភាពអាចរកបាន</p>
        <div className="flex flex-wrap gap-2">
      ...
      {areas.length > 0 && (
        <div className="space-y-2">
          <p className="text-sm font-medium text-gray-600">តំបន់</p>
          <div className="flex flex-wrap gap-2">
            <Badge
      ...
      {/* Special Toggles */}
      <div className="space-y-2">
        <p className="text-sm font-medium text-gray-600">តម្រងពិសេស</p>
        ...
        >
          <ShieldCheck className={`w-5 h-5 ${filters.verifiedOnly ? 'text-emerald-600' : 'text-gray-400'}`} />
          <span className={`font-medium ${filters.verifiedOnly ? 'text-emerald-700' : 'text-gray-600'}`}>
        ...
        <div 
          ...
          onClick={handleFamilyToggle}
        >
          <Baby className={`w-5 h-5 ${filters.familyFriendly ? 'text-pink-500' : 'text-gray-400'}`} />
          <span className={`font-medium ${filters.familyFriendly ? 'text-pink-700' : 'text-gray-600'}`}>
