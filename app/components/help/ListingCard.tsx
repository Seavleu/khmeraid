  ...
  if (compact) {
    return (
      <div 
        ...
        className="flex items-center gap-4 p-4 bg-white rounded-xl border-2 border-gray-200 hover:border-teal-300 hover:shadow-lg transition-all cursor-pointer"
      >
        <div className={`p-3 rounded-lg ${type.color}`}>
          <TypeIcon className="w-7 h-7" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-lg font-bold text-gray-900 truncate">{listing.title}</p>
          <p className="text-base text-gray-600 truncate font-medium">{listing.area}</p>
        </div>
        <Badge className={`${status.color} text-sm font-bold px-3 py-1.5`}>{status.label}</Badge>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <CardContent className="p-0">
        ...
        {/* Header */}
        <div className={`p-5 ${type.color} border-b-2`}>
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <div className="flex items-center gap-2 text-gray-600">
              ...
              <span className="text-base font-medium">{listing.area}</span>
            </div>
          </div>
              ...
              {listing.event_date && (
                <div className="bg-pink-50 border-2 border-pink-200 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-sm text-pink-700 font-medium">កាលបរិច្ឆេទ</p>
                  </div>
              ...
              {listing.organizer_name && (
                <div className="flex items-center gap-3 text-gray-700 bg-gray-50 p-3 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold">{listing.organizer_name}</p>
                  </div>
                </div>
              )}
              {listing.organizer_contact && (
                <div className="bg-pink-50 border-2 border-pink-200 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-sm text-pink-700 font-medium">ទំនាក់ទំនងអ្នករៀបចំ</p>
                  </div>
                  <a 
                    href={`tel:${listing.organizer_contact.replace(/[^0-9+]/g, '')}`}
                    className="text-xl font-bold text-pink-700 hover:text-pink-800 block"
          ...
          <div className="space-y-3">
            {listing.contact_phone && (
              <div className="bg-emerald-50 border-2 border-emerald-200 rounded-xl p-4">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <p className="text-sm text-emerald-700 font-medium">លេខទូរស័ព្ទ</p>
                      </div>
                <a 
                  href={`tel:${listing.contact_phone.replace(/[^0-9+]/g, '')}`}
                  className="text-2xl font-bold text-emerald-700 hover:text-emerald-800 block"
                  onClick={(e) => e.stopPropagation()}
                >
                  {listing.contact_phone}
                </a>
            ...
            {listing.contact_name && (
              <div className="flex items-center gap-3 text-gray-700 bg-gray-50 p-3 rounded-lg">
                <User className="w-5 h-5 text-gray-500" />
                  ...
                  <p className="text-base font-semibold">{listing.contact_name}</p>
                </div>
              </div>
            )}
            ...
            {listing.facebook_contact && (
              <a
                ...
                onClick={(e) => e.stopPropagation()}
                className="flex items-center gap-3 bg-blue-50 border-2 border-blue-200 p-3 rounded-lg hover:bg-blue-100 transition-all"
              >
                <Facebook className="w-5 h-5 text-blue-600" />
                <div>
                  <p className="text-xs text-blue-600 font-medium">ហ្វេសប៊ុក</p>
                  <p className="text-base font-semibold text-blue-700">ផ្ញើសារតាម Facebook</p>
                </div>
              </a>
            )}
            ...
            {listing.exact_location && listing.location_consent && (
              <div className="flex items-start gap-3 text-gray-700 bg-gray-50 p-3 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-base">{listing.exact_location}</p>
                </div>
              </div>
            )}
          </div>
            ...
            {listing.family_friendly && (
              <div className="flex items-center gap-3 text-pink-600">
                <Baby className="w-5 h-5" />
                  <span className="text-base font-semibold">សមស្របសម្រាប់គ្រួសារ</span>
              </div>
            )}
          </div>
          ...
          {listing.contact_phone && (
            <Button 
              onClick={handleCall}
              size="lg"
              className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-lg rounded-xl py-6 mt-2"
            >
